...
export class DocumentsController {
  ...
  async create(
    ...
    if (invalidAttributes.length) {
      ...
      throw new errors.ForbiddenException(
        `providing the properties: ${properties} on ${"Documents"} creation is forbidden for roles: ${roles}`
      );
    }
    // @ts-ignore
  ...
  async findMany(
    @common.Query() query: DocumentsWhereInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<Documents[]> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
  ...
  async update(
    ...
    if (invalidAttributes.length) {
      ...
      const roles = userRoles
        ...
        .join(",");
      throw new errors.ForbiddenException(
        `providing the properties: ${properties} on ${"Documents"} update is forbidden for roles: ${roles}`
      );
    }
    try {
      // @ts-ignore
